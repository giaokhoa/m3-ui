...
test.describe('Material 3 Card parity', () => {
  test('static variants keep Compose containers, shape and base elevations', async ({ page }) => {
    ...
    const cards = page.locator('.m3-card');
    await expect(cards).toHaveCount(3);

    const values = await cards.evaluateAll((elements) =>
      elements.map((element) => {
        const rootStyle = getComputedStyle(element as HTMLElement);
        const surface = element.querySelector<HTMLElement>('.m3-card__surface')!;
        const surfaceStyle = getComputedStyle(surface);
        const elevation = element.querySelector<HTMLElement>('.m3-elevation')!;
        return {
  ...
  test('uses latest interaction to animate filled and elevated hover elevations', async ({ page }) => {
    ...
    await expect(cards.nth(0).locator('.m3-elevation')).toHaveAttribute('data-elevation', 'level1');
    await expect(cards.nth(0).locator('.m3-elevation')).toHaveCSS('transition-duration', '0.12s');

    await cards.nth(1).hover();
    await expect(cards.nth(1).locator('.m3-elevation')).toHaveAttribute('data-elevation', 'level2');
  ...
  test('presses use full-surface ripple while elevation resolves to pressed state', async ({ page }) => {
    await openStory(page, 'components-card--clickable');
    const card = page.locator('.m3-card').first();
    ...
    await expect(card.locator('.m3-elevation')).toHaveAttribute('data-elevation', 'level0');
    await expect(card.locator('.m3-ripple__wave')).toHaveCount(1);
  ...
  test('disabled card colors match Compose alpha/composite behavior', async ({ page }) => {
    ...
    await expect(filled.locator('.m3-card__surface')).toHaveCSS('background-color', filledContainer);
    await expect(elevated.locator('.m3-card__surface')).toHaveCSS(
      'background-color',
      elevatedContainer,
    );
  ...
  test('moves keyboard focus indication between opacity and inset-ring modes', async ({ page }) => {
    ...
    const opacityRipple = cards.nth(0).locator('.m3-ripple');
    ...
    const insetRipple = cards.nth(1).locator('.m3-ripple');
    await expect(insetRipple).toHaveAttribute('data-inset-focus-visible', 'true');
    const ring = insetRipple.locator('.m3-ripple__focus-ring');
    const geometry = await ring.evaluate((element) => {
      const box = element.getBoundingClientRect();
      const surface = element.closest('.m3-card__surface')!.getBoundingClientRect();
      return {
  ...
  test('allows an explicit web role without forcing it on Compose parity defaults', async ({ page }) => {
    ...
    await expect(card).toHaveClass(/m3-card/);
  });
  ...
  test('supports a shape override while preserving card clipping', async ({ page }) => {
    ...
    const card = page.locator('.m3-card');
    await expect(card.locator('.m3-card__surface')).toHaveCSS('border-radius', '4px');
    await expect(card.locator('.m3-elevation')).toHaveCSS('border-radius', '4px');
  });
