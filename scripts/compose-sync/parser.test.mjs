...
test('normalizes supported AndroidX token expressions', () => {
  ...
  assert.deepEqual(normalizeTokenExpression('ShapeKeyTokens.CornerFull'), {
    kind: 'shape',
    value: 'full',
  });
...
test('parses generated const and inline token declarations in source order', () => {
...
test('fails loudly instead of guessing unsupported expressions', () => {
  assert.throws(
    () => normalizeTokenExpression('someFunction()'),
