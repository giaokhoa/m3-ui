...
Agents changing Material component/theme/token ownership must read `/.agents/skills/material3-parity/SKILL.md` first. Changes to this package additionally require `/.agents/skills/style-dictionary/SKILL.md`.
...
## Compose defaults are audit semantics, not a second runtime token layer

AndroidX Compose component `FooDefaults` APIs are useful evidence for Material semantic defaults, variants, state mappings, size matrices, behavior, and customization capability. They do not become a second source of truth in m3-ui.

For immutable component defaults, the intended web path is:
...
Do not add `FooDefaults.colors()`, `FooColors`, `Foo.tokens.ts`, or `Foo.defaults.ts` merely to restate immutable mappings already represented by canonical DTCG. Runtime TypeScript value objects/helpers are allowed only when JavaScript genuinely needs them for runtime behavior or a real web API customization boundary.

## Baseline theme data follows the same compiler rule

Immutable Material baseline light/dark schemes and static baseline typography mappings are design data and follow the same canonical DTCG -> Style Dictionary rule as immutable component defaults.

Current handwritten baseline serialization in the UI theme package is migration debt tracked by issue #150. It is not precedent for adding more handwritten foundation tables.

Runtime dynamic scheme generation from `sourceColor` remains a `ThemeProvider` responsibility because it depends on runtime input.
...
- mandatory Material component/theme/token entrypoint: `/.agents/skills/material3-parity/SKILL.md`;
