...
const modules = [
  ['md-comp-split-button-xsmall', 'xSmall'],
  ['md-comp-split-button-small', 'small'],
  ['md-comp-split-button-medium', 'medium'],
  ['md-comp-split-button-large', 'large'],
  ['md-comp-split-button-xlarge', 'xLarge'],
].map(([module, size]) => ({ module, root: `component.splitButton.size.${size}` }));
...
function pathFor(module, variable) {
  const names = {
    ...
    'leading-button-leading-space': 'leadingButtonLeadingSpace',
    'leading-button-trailing-space': 'leadingButtonTrailingSpace',
    'outer-corner-corner-size': 'outerCornerSizePercent',
    'trailing-button-icon-size': 'trailingIconSize',
    'trailing-button-inner-corner-selected-corner-size': 'trailingInnerSelectedCornerSizePercent',
    'trailing-button-leading-space': 'trailingButtonLeadingSpace',
    'trailing-button-trailing-space': 'trailingButtonTrailingSpace',
    'container-shape': 'containerShape',
    'inner-corner-corner-size': 'innerCornerSize',
    'inner-corner-hovered-corner-size': 'innerHoveredCornerSize',
    'inner-corner-pressed-corner-size': 'innerPressedCornerSize',
  };
  const name = names[variable];
...
for (const module of modules) {
  ...
  const response = await fetch(url, { headers: { 'user-agent': 'm3-ui-split-button-overlap-audit' } });
...
console.log(`Material Web Split Button overlap audit: modules=${modules.length} declarations=${results.length} reconciled=${results.length - pending.length} pending=${pending.length}`);
