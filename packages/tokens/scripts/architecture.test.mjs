...
test('Style Dictionary reads only canonical DTCG and emits reviewed platform artifacts', async () => {
  ...
  assert.deepEqual(
    ...
    ['button.css', 'elevation.css', 'ripple.css'],
...
test('foundation docs lock change-local interaction-effect contracts', async () => {
  const architecture = await readFile(
    new URL('docs/architecture/README.md', repoRoot),
    'utf8',
  );
  const elevation = await readFile(
    new URL('packages/ui/src/internal/elevation/README.md', repoRoot),
    'utf8',
  );
  const ripple = await readFile(
    new URL('packages/ui/src/internal/ripple/README.md', repoRoot),
    'utf8',
  );
...
test('Elevation and Ripple primitives do not serialize immutable design tokens in React', async () => {
  const elevation = await readFile(
    new URL('../ui/src/internal/elevation/Elevation.tsx', packageRoot),
    'utf8',
  );
  const ripple = await readFile(
    new URL('../ui/src/internal/ripple/Ripple.tsx', packageRoot),
    'utf8',
  );
  const button = await readFile(
    new URL('../ui/src/components/Button/Button.tsx', packageRoot),
    'utf8',
  );
  ...
  assert.doesNotMatch(
    ripple,
    /RippleRadiusDuration|StateLayerOpacityHover|RippleFocusRingOuterStrokeWidth/,
  );
...
test('legacy TypeScript elevation serialization is frozen to the existing migration allowlist', async () => {
  ...
  for (const file of await sourceFiles(uiSourceRoot)) {
    ...
    assert.ok(
      legacyElevationSerializerCallers.has(relative),
      `${relative} adds a new legacy getElevationBoxShadow caller; use <Elevation> + generated elevation.css instead`,
    );
  }

  assert.deepEqual(
    [...observed].sort(),
    [...legacyElevationSerializerCallers].sort(),
    'legacy elevation allowlist changed; migrations must remove the caller from this set in the same PR',
  );
});
