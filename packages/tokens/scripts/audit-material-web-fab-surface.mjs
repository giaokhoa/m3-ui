...
const modules = [
  ['md-comp-fab-surface', 'component.fab.surface'],
  ['md-comp-fab-branded', 'component.fab.branded'],
  ['md-comp-fab-surface-large', null],
  ['md-comp-fab-branded-large', null],
...
function pathFor(module, variable) {
  ...
  if (module.module === 'md-comp-fab-branded' && variable === 'indicator-outline-offset') {
    return `${module.root}.focusIndicatorOutlineOffset`;
  }
...
for (const module of modules) {
  ...
  const response = await fetch(url, { headers: { 'user-agent': 'm3-ui-fab-surface-overlap-audit' } });
...
console.log(`Material Web FAB surface overlap audit: modules=${modules.length} current=${current.length} reconciled=${current.length - pending.length} pending=${pending.length} deprecated=${results.length - current.length} deprecationOnlyModules=${deprecationOnly}`);
