...
| Menu | Open/close, option queries/selection, submenu helpers | `apps/storybook/visual/menu.visual.spec.ts` and `button-group.visual.spec.ts` cover focus entry, arrow navigation, disabled items, activation, Escape, focus restoration, outside dismissal, submenus, and RTL placement | Do not duplicate. The tester would mostly restate already-semantic Playwright flows. |
