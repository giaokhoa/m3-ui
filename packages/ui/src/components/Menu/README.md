...
`MenuSubmenu` is a Material wrapper around React Aria `SubmenuTrigger`. React Aria remains responsible for logical Arrow-key disclosure, hover delay, Escape, focus travel, nested overlay placement, and RTL locale direction. The wrapper must emit a real React Aria collection item as the submenu trigger rather than creating a second disclosure engine.
