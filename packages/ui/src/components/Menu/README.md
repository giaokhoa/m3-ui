...
`MenuSubmenu` is a Material wrapper around React Aria `SubmenuTrigger`. React Aria remains responsible for logical Arrow-key disclosure, hover delay, Escape, focus travel, nested overlay placement, and RTL locale direction. The wrapper must emit a real React Aria collection item as the submenu trigger rather than creating a second disclosure engine. Root menus inherit the active `ThemeProvider` target through React Aria's `UNSAFE_PortalProvider`; nested submenu popovers must continue using React Aria's root `PopoverGroupContext` portal rather than supplying a component-local `UNSTABLE_portalContainer`, preserving deterministic nested focus containment/restoration.
