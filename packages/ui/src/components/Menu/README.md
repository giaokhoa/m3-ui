...
`MenuSubmenu` is a Material wrapper around React Aria `SubmenuTrigger`. React Aria remains responsible for logical Arrow-key disclosure, hover delay, Escape, focus travel, nested overlay placement, and RTL locale direction. The wrapper must emit a real React Aria collection item as the submenu trigger rather than creating a second disclosure engine. Nested submenu popovers must use React Aria's root `PopoverGroupContext` portal rather than overriding `UNSTABLE_portalContainer`: the root Menu is already inside the active ThemeProvider portal, and preserving the RAC group is required for deterministic nested focus containment/restoration.
