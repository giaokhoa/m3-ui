...
describe('Material 3 chip defaults', () => {
  ...
  it('maps selectable state colors and borders without generated interaction colors', () => {
    ...
    const selected = getChipStyle('filter', {
      isDisabled: false,
      isSelected: true,
      interaction: null,
    });
    ...
    const disabledElevated = getChipStyle('elevatedFilter', {
      isDisabled: true,
      isSelected: false,
      interaction: null,
    });
    expect(elevatedFilterChipTokens.disabledUnselectedContainerColor).toBe('onSurface');
    expect(disabledElevated['--_chip-container-color']).toBe(
  ...
  it('ports expressive shapes, tonal leading color and compact spacing', () => {
    ...
    const selected = getChipStyle(
      'filter',
      { isDisabled: false, isSelected: true, interaction: null },
      { shapes: {} },
    );
    ...
    const pressed = getChipStyle(
      'filter',
      { isDisabled: false, isSelected: true, interaction: 'press' },
      { shapes: {} },
    );
  ...
  it('uses Material elevation transition selection', () => {
    expect(
      resolveChipElevation('elevatedAssist', {
        isDisabled: false,
        interaction: 'hover',
      }),
    ).toBe('level2');
    expect(
      resolveChipElevation('filter', {
        isDisabled: false,
        isSelected: false,
        interaction: 'hover',
      }),
