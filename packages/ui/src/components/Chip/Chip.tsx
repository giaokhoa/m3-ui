...
import type { ChipVariant } from '@m3/tokens/chip';
...
export interface SelectableChipProps
  ...
  shape?: ChipShapeValue;
  /**
   * Opts into the pinned expressive ChipShapes overload: 12px unselected,
   * full selected, and 8px pressed unless overridden.
   */
  shapes?: ChipShapes;
}
...
function ChipVisual({
  ...
  activeInteractions: readonly ChipInteraction[];
  previousInteraction: ChipInteraction | null;
  isDisabled: boolean;
  isSelected: boolean;
  ...
  const style = getChipStyle(variant, state, {
    shape,
    shapes,
  ...
  return (
    <span
      className="m3-chip__visual"
      data-selected={isSelected || undefined}
      data-expressive-shapes={shapes ? true : undefined}
      style={style}
    >
      <Elevation
        className="m3-chip__elevation"
        level={resolveChipElevation(variant, state)}
        style={{
      ...
      <span className="m3-chip__surface">
        <Ripple
          ...
          stateInteraction={latestChipStateLayerInteraction(
            activeInteractions,
            isFocusVisible,
          )}
        />
        <span className="m3-chip__content">
          {slots.avatar != null ? (
            <span aria-hidden="true" className="m3-chip__avatar">
              {slots.avatar}
            </span>
          ) : slots.leadingIcon != null ? (
            <span aria-hidden="true" className="m3-chip__leading-icon">
              {slots.leadingIcon}
            </span>
          ) : null}
          <span className="m3-chip__label">{label}</span>
          {slots.trailingIcon != null ? (
            <span aria-hidden="true" className="m3-chip__trailing-icon">
              {slots.trailingIcon}
            </span>
          ) : null}
        </span>
...
function useChipInteractions() {
  ...
  const start = (value: ChipInteraction) => {
    setActiveInteractions((active) => startChipInteraction(active, value));
  };
  const end = (value: ChipInteraction) => {
    setActiveInteractions((active) => endChipInteraction(active, value));
  };
    ...
    >
      {(renderProps) => (
        <ChipVisual
          ...
          activeInteractions={interactions.activeInteractions}
          previousInteraction={interactions.previousInteraction}
