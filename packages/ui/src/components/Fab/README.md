...
Material 3 floating action button family mapped to the canonical `@m3-ui/tokens` Style Dictionary graph, pinned AndroidX Compose behavior, and current Material Web public FAB adaptations.
...
- React Aria `Button` owns button semantics, keyboard/pointer interaction, and current pressed/hovered/focused state.
...
- Shared `Elevation` resolves the repository precedence `pressed > hovered > focused > default` from current RAC render state using the FAB's canonical normal/lowered semantic level set. FAB does not keep a component-local interaction history.
- Default elevation is level 3, hover is level 4, focus/press are level 3. `elevation="lowered"` maps level 1 with hover level 2.
- Shared Elevation owns the observable Material elevation transition history: incoming interactions use the 120ms incoming tween, hover exit uses the 120ms hovered-outgoing tween, and press/focus exit uses the 150ms outgoing tween.
- `useRipple().getPressProps()` consumes RAC PressEvents for wave geometry/lifecycle and chains caller handlers; Ripple state-layer/focus indication reads current RAC hover/focus-visible state directly.
...
Branded Extended FAB label state colors use RAC's current `data-focused`, `data-hovered`, and `data-pressed` states. Selector order preserves the repository precedence without reconstructing a `data-interaction` value in React.
...
Storybook and Playwright cover regular size geometry, all seven color families, branded and branded-extended forms, normal/lowered elevation, Extended size families, text-only and collapsed behavior, keyboard focus, interaction states, and the light/dark/dynamic theme matrix. The nine checked-in PNG baselines were generated by the repository's Linux Chromium runner in successful bootstrap run #502.
